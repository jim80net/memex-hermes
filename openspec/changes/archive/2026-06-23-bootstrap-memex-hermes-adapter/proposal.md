...
- Invoke a prebuilt `memex-hermes` binary via subprocess; do **not** re-implement the engine in Python. Per the own-binary architecture decision, this adapter builds and ships its OWN `bun build --compile` binary from the `jim80net/memex-hermes` release pipeline (importing `@jim80net/memex-core` as a library), rather than reusing `memex-claude`'s artifact — see `bin/memex` and `pyproject.toml [tool.memex-hermes.binary]`.
...
- Hermes v0.14.0 calls provider methods synchronously from the turn thread (no asyncio event loop drives providers). Write paths (`sync_turn`, `queue_prefetch`, `on_memory_write`) run on a daemon-thread bounded queue so they return immediately and never block the turn; read paths run synchronously within a bounded join.
...
- `hermes-memory-provider`: The Python `MemoryProvider` subclass that implements every Hermes lifecycle method by dispatching to the `memex-hermes` binary — synchronously for read paths, via a daemon-thread queue for non-blocking writes. Covers method-by-method semantics, return-shape contracts, the verification-spike gate for `on_memory_write`, and the `HERMES_HOME` propagation rule.
...
- **Distribution:** New PyPI package `memex-hermes`; ships its OWN bun-compiled binary via the `jim80net/memex-hermes` GitHub release pipeline (`bin/memex` downloads + SHA256-verifies it on first run), not `memex-claude`'s artifacts.
